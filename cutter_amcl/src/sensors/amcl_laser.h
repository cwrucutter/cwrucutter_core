...
typedef enum
{
  ...
  LASER_MODEL_LIKELIHOOD_FIELD_LANDMARK
} laser_model_t;
...
class AMCLLaser : public AMCLSensor
{
  ...
  public: void SetModelLikelihoodFieldLandmark(double z_hit,
                                               double z_rand,
                                               double sigma_hit,
                                               double max_occ_dist);
  ...
  private: static double LikelihoodFieldModel(AMCLLaserData *data, 
                                              pf_sample_set_t* set);
  // Determine the probability for the given pose
  private: static double LikelihoodFieldLandmarkModel(AMCLLaserData *data, 
                                                      pf_sample_set_t* set);
  ...
  //private: double Cone2LaserAngle(double robot_x,double robot_y,double robot_psi,double cone_x,double cone_y);
  
//  private: double LaserConeError(double distance_expect,double distance_laser_real);
  
  //private: double Cone2Distance(double robot_x,double robot_y,double cone_x,double cone_y);
};
